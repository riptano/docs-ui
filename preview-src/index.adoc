...
[.rolename]
== Level 1 Section Title (w/ custom role)
...
.With header row, footer row, and hover striping
...
.Unconstrained SVG image
image::multirepo-ssg.svg[]

.Constrained SVG image w/ alt text
image::multirepo-ssg.svg[Multirepo SSG,300,opts=interactive]

.Jpeg Image from a remote URL
